...
class BiGRU(nn.Module):
    ...
    def forward(self, text, text_length):
        ...
        bigru_out, _ = torch.max(bigru_out, dim=1)
...
class BiLSTM(nn.Module):
    ...
    def forward(self, text, text_length):
        ...
        #         query = self.dropout_layer(bilstm_out)
        #         atn_bilstm_out, alpha_n = self.attention_net(bilstm_out, query)
        #         return atn_bilstm_out
        #     elif self.atn_layers == 'w':
        #         self.attention_net_with_w(bilstm_out, final_hidden_state)

        # [LSTM](https://zhuanlan.zhihu.com/p/79064602)
        # 取最常序列时刻的输出层, [bs, seq_len, dim] -> [bs, dim]
        bilstm_out_max, _ = torch.max(bilstm_out, dim=1)
        # bilstm_out = bilstm_out_max.unsqueeze(1)
        if self.cfg.MODEL.TEXTUAL_MODEL.WORDS:

            #return bilstm_out, text
            ...
            return bilstm_out_max, bilstm_out

        return bilstm_out_max
